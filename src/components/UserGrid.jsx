    ...
    <div className="w-full pb-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-8">
        {users.map((user, i) => (
          <motion.div
            ...
            className="glass-panel theme-text-primary p-4 flex flex-col items-center justify-center cursor-pointer minecraft-corners voxel-shadow text-center group theme-panel-subtle border border-transparent border-t-2 border-t-transparent hover:border-t-brand-neon-blue transition-all h-full"
          >
            <div className="relative mb-3 flex-shrink-0">
              <img 
                src={user.avatar_url} 
                alt={user.login} 
                className="w-16 h-16 rounded-full border-2 theme-card-border group-hover:border-brand-neon-blue transition-colors z-10 relative"
              />
            ...
            <h3 className="theme-text-primary text-sm font-semibold break-words w-full truncate px-1">
              {user.login}
            </h3>
          </motion.div>
        ))}
      ...
      {hasMore && (
        <div className="flex flex-col items-center mt-8 gap-4">
          ...
          <p className="theme-text-muted text-center text-sm max-w-xl">
